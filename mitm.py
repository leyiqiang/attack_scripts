...
def do_replay_tcp(client_sock, server_sock, cfg):
    ...
    while True:
        ...
        try:
            ...
            if packet.startswith('\x16\x03'):  # SSL/TLS Handshake.
                ...
                if not (cfg.cert and cfg.key):
                    print color("[!] SSL/TLS handshake detected, provide a server cert and key to enable interception.",
                                1)

                else:
                    print color('------------------ Wrapping sockets ------------------', 2)
                    client_sock = ssl.wrap_socket(client_sock, server_side=True, suppress_ragged_eofs=True,
                                                  certfile=cfg.cert.name, keyfile=cfg.key.name)
                    ...
                    if (cfg.clientcert and cfg.clientkey):
                        server_sock = ssl.wrap_socket(server_sock, suppress_ragged_eofs=True,
                                                      certfile=cfg.clientcert.name, keyfile=cfg.clientkey.name)
                    else:
                        server_sock = ssl.wrap_socket(server_sock, suppress_ragged_eofs=True)
        except:
            pass
        ...
        try:
            if client_sock in receiving:
                ...
                server_sock.send(data_out) # TODO replay
            ...
            if server_sock in receiving:
                ...
                client_sock.send(data_in) # TODO replay
...
def do_replay_udp(relay_sock, server, cfg):
    client = None
...
def handle_tcp_client(client_sock, target, cfg):
    ...
    if cfg.replay:
        do_replay_tcp(client_sock, server_sock, cfg)
    else:
...
def create_server(relay, cfg):
    ...
    if proto == 'tcp':
        ...
        serv.bind((cfg.listen, lport))
        if cfg.replay:
            thread = Thread(target=do_replay_udp, args=(serv, (rhost, rport), cfg))
        else:
